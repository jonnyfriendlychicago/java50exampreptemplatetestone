...
<body>
	...
	<div id=main class="card">
		<h3><c:out value="${student.studentName}"></c:out></h3>
...
<%-- 
		...
		<div id=list class="card">
			<h3>Teacher List for this Student</h3>
<%-- 			
			<c:choose>
...
--%>
			<table class="table table-striped table-dark">
				...
				<tbody>
					<c:forEach var="record" items="${assignedTeachers}">
						<tr>
							<td>${record.id}</td>
							<%-- <td><a href="/store/teacher/${record.id}">${record.teacherName}</a></td> --%>
							...
							<td>
								...
								<form action="/removeTeacherStudentJoin" method="post">
								    ...
								    <input type="hidden" name="teacherId" value="${record.id}"  />
		...
		<div id=form class="card">
			<h2>Add a Teacher:</h2>
			<%-- <form action='/store/student/${student.id}' method='post' > --%>
			<form action='/student/${student.id}' method='post' >
				...
				<div class="form-group">
					...
					<!-- JRF: above left out... this is a JSTL form thing -->
						...
						<c:forEach var="record" items="${unassignedTeachers}">
							<!--- Each option VALUE is the id of the dojo --->
							<option value="${record.id}" >
							<!-- path="studentMdl"> -->
								<!--- This is what shows to the user as the option --->
								<c:out value="${record.teacherName}" />
							</option>
						</c:forEach>
					</select>
				</div>
